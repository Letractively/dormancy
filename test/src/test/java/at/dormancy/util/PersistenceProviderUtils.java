...
public class PersistenceProviderUtils {
	...
	protected static final String OPEN_JPA_COLLECTION = "org.apache.openjpa.util.ProxyCollection";
	protected static final String OPEN_JPA_LIST = "org.apache.openjpa.util.DelayedArrayListProxy";
	protected static final String OPEN_JPA_SET = "org.apache.openjpa.util.DelayedHashSetProxy";
	protected static final String OPEN_JPA_MAP = "org.apache.openjpa.util.ProxyMap";

	protected static final Class<?> PERSISTENT_COLLECTION = ClassLookup.find(
			JPA_ECLIPSE_LINK_COLLECTION, HIBERNATE_4_COLLECTION, HIBERNATE_3_COLLECTION, OPEN_JPA_COLLECTION)
			.orThrow(new IllegalStateException("No supported persistence provider found")).get();
	protected static final Class<?> PERSISTENT_LIST = ClassLookup.find(
			JPA_ECLIPSE_LINK_LIST, HIBERNATE_4_LIST, HIBERNATE_3_LIST, OPEN_JPA_LIST)
			.orThrow(new IllegalStateException("No supported persistence provider found")).get();
	protected static final Class<?> PERSISTENT_SET = ClassLookup.find(
			JPA_ECLIPSE_LINK_SET, HIBERNATE_4_SET, HIBERNATE_3_SET, OPEN_JPA_SET)
			.orThrow(new IllegalStateException("No supported persistence provider found")).get();
	protected static final Class<?> PERSISTENT_MAP = ClassLookup.find(
			JPA_ECLIPSE_LINK_MAP, HIBERNATE_4_MAP, HIBERNATE_3_MAP, OPEN_JPA_MAP)
			.orThrow(new IllegalStateException("No supported persistence provider found")).get();
	...
	/**
	 * Returns {@code true} if OpenJPA collections are found in the classpath.
	 *
	 * @return {@code true} if OpenJPA is available, {@code false} otherwise
	 */
	public static boolean isOpenJpa() {
		return getPersistentCollectionClass().getName().equals(OPEN_JPA_COLLECTION);
	}
